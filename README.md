...
<agent_name>.sources.<source_name>.duplicatedEventsProcessor.algorithm = java_hashcode
...
It compares new Events with last events. A set of last Event hashes is maintained, the size of this set can be configured with the "size" parameter, the default size is 1000.

An Event's hash is calculated by default both from its headers (disabled if "header" parameter is set to false) and body (disabled if "body" parameter is set to false).

The list of hashes is persisted into disk, this allows to maintain the list in case agent is restarted. File to persist hashes can be configured by "path" parameter.

The hashing algorithm is selected by default based on the size of the last event hashes list. If 1000 or less, it defaults to Java's `hashCode()` implementation.
If the list is bigger than this, it will use MD5 hashing, to avoid any collision rates. To override the used algorithm, you can use the "algorithm" parameter, which can be set to either "md5" or "java\_hashcode". The switching to MD5 is required to avoid hash collisions.

Consult the table below to see if you might be affected by hash collisions, if using the `java_hashcode` algorithm:

| Processor size | Hash collision probability |
| -------------- | -------------------------- |
| 100            | < 0.01%                    |
| 1 000          | 0.011%                     |
| 10 000         | 1.1%                       |
